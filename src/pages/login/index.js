...
function Login() {
   ...
   const valueEmail = (e) => {
      ...
      setBoxcolor(false), setBoxpending(true);
   };

   const valuePassword = (e) => {
      setPassword(e.target.value);
      setBoxcolor_(false), setBoxpending_(true);
   };
   ...
   const handleLogin = () => {
      ...
      axios
         ...
         .then((res) => {
            console.log(res.data.data), Cookies.set("id", res.data.data.id);
            Cookies.set("role", res.data.data.role);
            Cookies.set("token", res.data.data.token);
            toast.success("Success Login");
            return dispatch(
               authActions.userThunk(res.data.data.token, () => {
                  setTimeout(() => {
         ...
         .catch((err) => {
            toast.error(err.response.data.status),
               setBoxcolor(false),
               setBoxpending(false);
            setBoxcolor_(false), setBoxpending_(false);
         });
   };
            ...
            <div className={css.content_right}>
                  ...
                  <div
                     className={
                        boxcolor_
                           ? css.password_box
                           : boxpending_
                           ? css.password_box_blue
                           : css.password_box_red
                     }
                  >
                     <input
                        type={type}
               ...
               <div className={css.already}>
                  <p>
                     ...
                     <Link href={"/resetpassword"}> Reset now</Link> Or
